...
type TVerificationStart = { questions: string[], images: TImage }

const VerificationStart = React.forwardRef<HTMLDivElement, TVerificationStart>(
  ({ questions, images }, ref: ForwardedRef<HTMLDivElement>) => {
...
const handleSkip = useCallback((event: React.MouseEvent) => {
  ...
  }, [currentCategory, shuffledImages, status])
  ...
  const checkQuestionValidity = Object.values(shuffledImages).some((img) => img.category === currentCategory);
    ...
    if (selectedImagesCategoryIsValid && checkQuestionValidity) {
        setStatus(STATUS.SUCCESS)
      }
    }
